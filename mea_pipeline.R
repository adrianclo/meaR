...
meaTable_dir = "ADD/HERE/YOUR/DIRECTORY/TABLE"
files_dir = "ADD/HERE/YOUR/DIRECTORY/FILES"
suffix = NULL

meaTable = create_meaTable(meaTable_dir, meaTable_sheet)

compiler(meaTable = meaTable, files_dir = files_dir) %>%
    region_filter() %>%
    layer_filter(specific = "upper", fileSuffix = suffix)  %>%
    ...
    norm_prop(fileSuffix = suffix, export = TRUE) -> ml; beep(2)
